...
У попередньому посібнику ми створили чергу завдань. Припущенням за базу черги завдань є те, що кожне завдання доставляється рівно одному робітнику. У цій частині ми зробимо зовсім інше - ми доставимо повідомлення до кількох споживачів. Цей шаблон відомий як "публікація/підписка".

Для ілюстрації цього шаблону ми побудуємо просту систему журналювання. Вона складатиметься з двох програм - перша буде випускати повідомлення журналу, а друга отримувати та друкувати їх.

У нашій системі журналювання кожна активна копія програми отримувача отримуватиме повідомлення. Таким чином, ми зможемо запустити один отримувач та спрямувати журнали на диск; і в той же час ми зможемо запустити ще один отримувач і бачити журнали на екрані.

Фактично, опубліковані повідомлення журналу будуть розіслані до всіх отримувачів.

## Обмінники

У попередніх частинах посібника ми надсилали та отримували повідомлення до і з черги. Тепер настав час представити повну модель обміну повідомленнями у Rabbit.

Давайте швидко пройдемося по тому, що ми розглянули в попередніх посібниках:

- ***Продюсер*** - це користувацька програма, яка надсилає повідомлення.
- ***Черга*** - це буфер, що зберігає повідомлення.
- ***Споживач*** - це користувацька програма, яка отримує повідомлення.

Основна ідея в моделі обміну повідомленнями у RabbitMQ полягає в тому, що виробник ніколи не надсилає жодних повідомлень безпосередньо до черги. Насправді, дуже часто виробник навіть не знає, чи буде повідомлення доставлено до будь-якої черги взагалі.

Замість цього продюсер може надсилати повідомлення лише до ***обмінника***. Обмінник - це дуже проста річ. З одного боку він отримує повідомлення від продюсерів, а з іншого він направляє їх до черг. Обмінник повинен точно знати, що робити з отриманим повідомленням. Чи потрібно його додати до певної черги? Чи потрібно додати його до декількох черг? Або ж він має бути відкинутий. Правила для цього визначаються ***типом обмінника***.
...
Існує кілька типів обмінників: `direct`, `topic`, `headers` та `fanout` (розгалуження). Ми сконцентруємося на останньому - fanout. Давайте створимо обмінник цього типу і назвемо його `logs`:

```cs
channel.ExchangeDeclare("logs", ExchangeType.Fanout);
```

Обмінник fanout дуже простий. Як ви, ймовірно, вже здогадалися з назви, він просто транслює всі отримані повідомлення до всіх черг, які він знає. І саме це нам і потрібно для нашого журналування.

> ***Список обмінників***
>
> Для перегляду обмінників на сервері ви можете запустити корисну команду `rabbitmqctl`:
> ```
> sudo rabbitmqctl list_exchanges
> ```
>
> У цьому списку будуть деякі обмінники типу `amq.*` та обмінник за замовчуванням (без назви). Вони створюються за замовчуванням, але ймовірно, вам не знадобиться їх використовувати на даний момент.